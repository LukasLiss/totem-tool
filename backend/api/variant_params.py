...
# Bounds for the client-supplied discovery watchdog (seconds); mirrors
# views/variants.py so both endpoints clamp identically.
MIN_TIMEOUT_S = 1.0
MAX_TIMEOUT_S = 300.0
...
def parse_timeout(source) -> float:
    """Seconds for the discovery watchdog, clamped server-side.

    The watchdog can no longer be disabled: ``timeout_s <= 0`` used to turn it
    off entirely, which let a single request pin a worker indefinitely.
    """
    ...
    return min(max(timeout_s, MIN_TIMEOUT_S), MAX_TIMEOUT_S)
