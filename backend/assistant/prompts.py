...
def build_system_prompt(
    ...
    if active_mode == "teach":
        prompt_lines.extend([
            ...
            "   - When the user asks how to perform an action in the UI, where to find a feature, or requests a walkthrough (e.g. 'Where do I select event logs?', 'How do I create a dashboard in the UI?', 'Show me where to run conformance', 'Walk me through discovering a model', 'Guide me...'):",
            "   - Invoke the `highlight_element` tool with a multi-step sequence `steps: [{\"tour_id\": \"...\", \"label\": \"...\"}, ...]` (2 to 5 steps) of valid `data-tour-id` elements so the user can step through the interactive guided tour.",
            ...
            "4. Do NOT perform unrequested mutating operations in Teach Mode.",
            "",
        ])
    else:  # Act Mode
        prompt_lines.extend([
            ...
            "   - `VariantsComponent`: Process Execution Paths (x: 0, y: 2, w: 6, h: 5)",
            "   - `OCDottedChartComponent`: Time-Series Throughput & Latency (x: 6, y: 2, w: 6, h: 5)",
            "   - `NewOCDFGComponent`: Object-Centric Directed Follows Graph (x: 0, y: 7, w: 8, h: 6)",
            "   - `NumberofEventsComponent`: Event Volume & Activity Metrics (x: 8, y: 7, w: 4, h: 6)",
