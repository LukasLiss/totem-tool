...
| duplicate_event_ids | `discover_oc_petri_net_polars` | `KeyError` | pm4py OCPN vs duplicate ids — version-dependent (2.7.17.x raises, newer may not) |
| unicode_names | DuckDB `find_variants` | `UnicodeEncodeError` | ASCII assumption in variants signature |

As of the last verified run (CI / pm4py 2.7.17.x): **186 passed, 13 xfailed, 0 failed**
(`pytest tests/edge_cases/`). The 13 xfailed test instances collapse to the 5 distinct root
causes above — the `empty` import bug alone accounts for 7 of them, since it blocks every
Polars-path miner.

> Note: on newer pm4py (2.7.22.4+) the `duplicate_event_ids` OCPN xfail entry may
> report as XPASS-failure locally. That is expected — CI is the source of truth, and
> the entry should remain until pm4py is pinned in `totem_lib/pyproject.toml`.
