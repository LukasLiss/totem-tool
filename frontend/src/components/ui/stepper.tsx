 ...
 * Stepper component.
 *
 * Adapted from the ReUI open-source component library (Radix "base" variant):
 *   https://reui.io/components/stepper
 *   Source: https://github.com/keenthemes/reui
 *   Licensed under the MIT License (c) Keenthemes.
 *
 * Modifications for this project: removed ReUI's theme-specific `style-*`
 * Tailwind variants, repointed the `cn` import to `@/lib/utils`, and added a
 * `data-[state=completed]` color to the separator.
 */
...
function StepperIndicator({ children, className }: React.ComponentProps<"div">) {
  ...
  return (
    ...
    >
      <div className="absolute">{override ? override : children}</div>
    </div>
  );
}
...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "bg-muted data-[state=completed]:bg-primary m-0.5 rounded-full transition-colors group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1 group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5",
        className
      )}
    />
  );
