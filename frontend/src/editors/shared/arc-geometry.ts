...
/**
 * Node-shape-agnostic floating-arc geometry shared by the OCPN and OC-DFG
 * editors: arcs attach to the node border on the straight line towards their
 * next path point (circle or rectangle border), optionally routed through
 * draggable bend points. Keeping the math in one place guarantees the drawn
 * edge, the connection-line preview and the editors' bend-point insertion
 * always agree — and that both editors render identically.
 */
 ...
 * node: circle border when `circle`, rectangle border otherwise.
 */
