...
\begin{tikzpicture}[
  font=\scriptsize,
  box/.style={draw, rounded corners=1pt, align=center, inner sep=2.5pt, minimum height=13pt},
...
\node[store, right=5.5mm of src] (db) {relational OCEL\\ \tiny 5 tables + indexes};
\node[comp, right=5.5mm of db] (sql) {discovery\\ \tiny P1--P4 in SQL};
\node[box, below=5mm of src] (stream) {event stream\\ \tiny append batches};
\node[store, below=5mm of db] (state) {summaries\\ \tiny DF, lifetimes, pairs};
\node[outbox, below=5mm of sql] (models) {models\\ \tiny OC-DFG, TOTeM, var.};
\draw[arr] (src) -- node[above]{\tiny import} (db);
\draw[arr] (db) -- (sql);
\draw[arr] (sql) -- (models);
\draw[arr] (stream) -- node[above]{\tiny $\Delta$} (state);
\draw[arr] (stream) -- (db);
\draw[arr] (state) -- node[above]{\tiny refresh} (models);
\end{tikzpicture}
...
Log & $|E|$ & $|O|$ & $|R|$ & types & JSON & XML & SQLite & ours \\
\midrule
...
The decisive variable is trace length (Figure~\ref{fig:ablation-tracelen}): holding the event count fixed at $3{\cdot}10^5$ and growing $L$ from 10 to 300 leaves the optimized formulations flat (OC-DFG 0.33--0.37\,s, TOTeM 0.28--0.37\,s---if anything, slightly faster, as fewer objects mean fewer groups), while the naive intermediates grow as predicted by Table~\ref{tab:complexity}: the self-join needs 49.5\,s at $L{=}100$ ($146\times$ the window plan, at $15\times$ the memory) and at $L{=}300$ fails after 181\,s, having exhausted 28\,GB of disk spill space; lazy aggregation grows linearly in $L$ to $13\times$ the eager plan at $L{=}300$. Long lifecycles are common (machines, containers, customer objects), so these rewrites are not micro-optimizations but the difference between feasible and infeasible---and they come for free once discovery is formulated relationally, since both exploit standard engine machinery.
\subsection{RQ3: Incremental Maintenance}
\label{sec:eval-inc}

\begin{figure*}
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_incremental.pdf}
\caption{Keeping models fresh while the log grows: per-batch maintenance and model refresh vs.\ full recomputation (log scale).}
\label{fig:incremental}
\end{figure*}

We replay each synthetic log as a base of 50\% of its events followed by ten appended batches of 5\% each, in timestamp order---emulating a system that ingests events continuously---and compare keeping the model fresh via the maintained summaries of Section~\ref{sec:incremental} against recomputing it (Figure~\ref{fig:incremental}).

Two costs matter. \emph{Maintenance} is the extra work on append beyond storing the events: subtracting measured plain-ingestion time, it amounts to $2.6$--$4.6\,\mu$s per event for the OC-DFG and $6.8$--$10.6\,\mu$s per event for TOTeM, constant per event as the log grows from $3{\cdot}10^5$ to $3{\cdot}10^6$ events---confirming that per-batch cost depends on the batch, not the history. \emph{Refresh}---materializing the current model from the summaries---costs 8\,ms for the OC-DFG and 15--39\,ms for TOTeM, versus 0.40--3.30\,s for full recomputation at these sizes: $50$--$412\times$ ($28$--$82\times$ for TOTeM), with the factor growing linearly in log size since recomputation scans the full log while refresh reads only aggregates. After every batch, the maintained model is bit-identical to the recomputed one (Proposition~\ref{prop:equiv}, verified per batch in the test suite). In short, \sys turns "always-fresh process models" from a periodic batch job whose cost grows with history into a constant-time-per-event side effect of ingestion.
