"""
Performance and memory evaluation: OCDFG.from_ocel() (Polars) vs OCDFGDb.from_ocel_db() (DuckDB).
...
Measures four phases per dataset, each timed and memory-profiled independently:
  ...
  OC-DFG algorithm | OCDFG.from_ocel(ocel)   | OCDFGDb.from_ocel_db(db)

Run from the totem_lib/ directory:
    python evaluation/ocdfg_performance.py
"""
...
RESULTS_FILE = SCRIPT_DIR / "OCDFG_PERFORMANCE_RESULTS.md"
...
def evaluate_dataset(dataset: dict) -> dict | None:
    ...
    print("  [polars] OCDFG.from_ocel ...", end=" ", flush=True)
    _, alg_p_t, alg_p_mem = _measure(OCDFG.from_ocel, ocel)
    print(f"{alg_p_t}s  {alg_p_mem} MB")

    print("  [db]     OCDFGDb.from_ocel_db ...", end=" ", flush=True)
    _, alg_db_t, alg_db_mem = _measure(OCDFGDb.from_ocel_db, db)
...
HEADER = (
    ...
    "| OCDFG Polars (s) | OCDFG Polars RAM (MB) "
    "| OCDFG DB (s) | OCDFG DB RAM (MB) |"
)
SEPARATOR = "|---|---|---|---|---|---|---|---|---|---|---|"
ROW_FMT = (
...
def append_results(results: list[dict]) -> None:
    if not RESULTS_FILE.exists():
        with open(RESULTS_FILE, "w") as f:
            f.write("# OC-DFG Algorithm Performance Results\n\n")
