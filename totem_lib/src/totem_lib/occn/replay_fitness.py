...
class OCCNReplayUnitResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.stopping_activity is not None and (
            not isinstance(self.stopping_activity, str)
            or not self.stopping_activity
        ):
            raise ValueError("stopping_activity must be a non-empty string or None")
