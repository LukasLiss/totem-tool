...
def _ab_executions(prefix, n):
    ...
    for i in range(n):
        oid = f"{prefix}{i}"
        events += [event(f"{prefix}a{i}", "A", 0, [oid]), event(f"{prefix}b{i}", "B", 100, [oid])]
...
def test_relative_ignores_volume_but_absolute_does_not():
    ...
    assert n_gram_distance_absolute(actual, simulated) == 1 / 3  # count mismatch penalized
    assert n_gram_distance_relative(actual, simulated) == 0.0    # same relative frequencies
