...
def test_cardinality_distribution_and_distance():
    ...
    assert card[("Item", "Order")] == {
        1: 2
    }  # each of the 2 Items co-occurs with 1 Order
...
def test_object_lifecycle_distribution_and_distance():
    actual = make_ocel(
        [event("e1", "A", 0, ["o1"]), event("e2", "B", 100, ["o1"])],
        [obj("o1", "Order")],
    )
    assert object_lifecycle_distribution(actual)["Order"] == {
        "lengths": [2],
        "times_s": [100],
    }

    simulated = make_ocel(
        [event("e1", "A", 0, ["o1"]), event("e2", "B", 300, ["o1"])],
        [obj("o1", "Order")],
    )
...
def test_graph_edit_distance():
    a = make_ocel(
        [event("e1", "A", 0, ["o1"]), event("e2", "B", 100, ["o1"])], [obj("o1", "T")]
    )
    b = make_ocel(
        [event("e1", "A", 0, ["o1"]), event("e2", "C", 100, ["o1"])], [obj("o1", "T")]
    )
