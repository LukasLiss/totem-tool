...
def test_absolute_event_distribution_uniform_shift():
    ...
    assert absolute_event_distribution_distance(actual, simulated) == pytest.approx(
        1.0
    )  # every event shifted 1h
...
def test_circadian_event_distribution_same_weekday_hour_shift():
    ...
    assert circadian_event_distribution_distance(actual, simulated) == pytest.approx(
        2 / 7
    )  # one of 7 weekdays differs by 2h
...
def test_relative_event_distribution_relative_to_case_start():
    actual = make_ocel(
        [event("e1", "A", 0, ["o1"]), event("e2", "B", HOUR, ["o1"])], [obj("o1", "T")]
    )
    simulated = make_ocel(
